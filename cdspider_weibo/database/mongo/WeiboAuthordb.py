...
class WeiboAuthorDB(Mongo, BaseWeiboAuthorDB):
    ...
    __tablename__ = 'weiboAuthor'

    incr_key = 'weiboAuthor'

    def __init__(self, connector, table=None, **kwargs):
        super(WeiboAuthorDB, self).__init__(connector, table = table, **kwargs)
        ...
        if not 'addWeiboAuthor' in indexes:
            collection.create_index('addWeiboAuthor', name='addWeiboAuthor')
    ...
    def insert(self, obj = {}):
        ...
        _id = super(WeiboAuthorDB, self).insert(setting=obj)
        return obj['uuid']

    def update(self, id, obj = {}):
        obj['utime'] = int(time.time())
        return super(WeiboAuthorDB, self).update(setting=obj, where={'uuid': int(id)}, multi=False)

    def update_many(self, obj = {},where=None):
        if where=={} or where==None:
            return
        obj['utime'] = int(time.time())
        return super(WeiboAuthorDB, self).update(setting=obj, where=where, multi=True)
    ...
    def delete(self, id, where = {}):
        ...
        return super(WeiboAuthorDB, self).update(setting=obj, where=where, multi=False)
